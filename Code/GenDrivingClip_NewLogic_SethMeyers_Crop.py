        ...
        for i in range(len(referenceClips)):
            ...
            for nameVideo in referenceClips[i]:
                ...
                if not os.path.exists(pathVideo):
                    ...
                    exit()
                TempVideoList.append(pathVideo)
            clipsTempVideo = [VideoFileClip(file) for file in finalVideoList]
            TempVideo_clip = concatenate_videoclips(clipsTempVideo, method="compose")
            TempVideo_clip.write_videofile('/media/anlab/data-2tb/ANLAB_THUY/ModuleDrivingClipStarpower/Temp/TempVideo/'+newName+'.mp4',fps=30)
